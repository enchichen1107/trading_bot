...
api_secret = get_secret(secret_name = "forBotTest1", region_name = "us-east-1")            
api_secrets = json.loads(api_secret)           

api_key1 = api_secrets['api_key1']
secret_key1 = api_secrets['secret_key1']


# connect to binance api
client = Client(api_key = api_key1, api_secret = secret_key1, tld = "com", testnet = True)
...
class breakout_bot():
    ...
    def __init__(self, symbol, interval, units, now, rollx, volx, position = ""):
        ...
        # set close bot time
        self.end = now + timedelta(hours=0.25)
        self.rollx = rollx
        self.volx = volx
        self.loss = 0
...
class data_collector():
    ...
    def __init__(self, now, info):
        ...
        self.end = now + timedelta(hours=0.3)
        self.api_key = api_key1
        self.secret_key = secret_key1
    ...
    def stream_data(self, msg):
        ...
        if (msg["e"]=='executionReport') and (msg["x"]=='TRADE') and (msg["X"]=='FILLED'):
            ...
            postgres_insert_query = """ INSERT INTO trade_record1 (transaction_time, symbol, side, price) VALUES (%s,%s,%s,%s)"""
            ...
            for item in msg["B"]:
                ...
                postgres_insert_query = """ INSERT INTO asset1 (update_time, symbol, amount) VALUES (%s,%s,%s)"""
    ...
    def alert_mail(self, mail_secret):
        ...
        content["subject"] = "Alert from bot1"  
        content["from"] = "enchichen.md10@nycu.edu.tw"  
        content["to"] = "enchichen.md10@nycu.edu.tw" 
        content.attach(MIMEText("You have better check out trading bot1")) 
...
if __name__ == '__main__':
    ...
    # initialize bot1's trading
    bot1 = breakout_bot(symbol = symbol, interval = interval, now = now, rollx = rollx,
                        volx = volx, units = units, position = position)
    
    # initialize bot1's trading data collector
    now = datetime.utcnow()
    bot1_data = data_collector(now = now, info = info)
    
    # start process
    p1 = Process(target=bot1.start_trading)
    p1.start()
    p2 = Process(target=bot1_data.start_collecting)
    p2.start()
    p1.join()
    p2.join()
